...
for (let heroe of superHeroes) {
    heroesContent += `<div class="heroe">
        <h2>${heroe.name}</h2>
        <img src="${heroe.img}" style="width:40vw"><br>
        <p>Поставьте герою оценку от 1 до 10</p>
        <input type="number" min="1" max="10" class="rating" required>
        <div>${heroe.universe}</div>
        <div>${heroe.alterEgo}</div>
        <div>${heroe.occupation}</div>
        <div>${heroe.friends}</div>
        <div>${heroe.superpowers}</div>
        <div>${heroe.about}</div>
        </div>`;
}
...
button.addEventListener("click", () => {
    ...
    localStorage.setItem("batmanRate", rates[0].value);
    localStorage.setItem("supermanRate", rates[1].value);
    localStorage.setItem("ironmanRate", rates[2].value);
